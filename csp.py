...
#___________________________________________________________________________________________________________________        
        ...
        for var in self.variables:
            for varx in self.variables:
                if var!=varx and (var, varx) not in self.seen and reversed(tuple((var,varx))) not in self.seen:
                    self.seen.add((var, varx))
                    self.weight[var,varx]=1 
...
def dom_wdeg(assignment, csp):
    a = 1000000
    sum = dict()   #dictionnaire des variables avec leurs valeurs
    result=None   
    ...
    for var in csp.variables:
        if var not in assignment:
            sum[var]=0   # initialiseation 
            for neighbor in csp.neighbors[var]:
                if neighbor not in assignment:
                    sum[var]+=csp.weight[(var, neighbor)]     #add weigth de la variable a la sum
            # domain_size nbr d'element dans chaque domaine
            domain_size = len(csp.curr_domains[var])   
            if sum[var]==0:
                continue
            if a > domain_size/sum[var]:
                a = domain_size/sum[var]
                result = var     
        else:
            continue
    if result:
...
def solution_search(csp, select_unassigned_variable=first_unassigned_variable,
